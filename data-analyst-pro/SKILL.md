...
**灰色地带裁决**：
- 当需求可路由到多个技能时 → 看任务主目标：目标是"理解数据含义/找规律"为本技能；目标是"自动批量处理文件"为 office-auto-pro
- 当需求超出本技能范围时 → 如果需要操作PPT软件制作幻灯片 → 路由到 ppt-pro；如果需要操作Excel宏/VBA → 路由到 script-gen-pro
- 当用户给了一份原始表格说"帮我看看" → 先判断是否需要数据清洗（若需要→建议先走office-auto-pro），再进入本技能分析
...
### 四、工具/软件类
`Excel` `表格处理` `电子表格` `WPS表格` `数据透视` `透视表` `VLOOKUP` `公式` `函数` `条件格式` `数据验证` `筛选` `排序` `分类汇总`

### 五、数据形态类
`数据` `数据集` `原始数据` `数据源` `数据表` `CSV` `数据文件` `导入数据` `导出数据` `数据清洗` `数据整理` `去重` `缺失值` `数据校验` `数据质量`
...
### 七、统计分析类
`统计` `数理统计` `描述统计` `平均值` `中位数` `标准差` `方差` `占比` `百分比` `累计` `求和` `极值` `最大值` `最小值` `分位数` `四分位数`
