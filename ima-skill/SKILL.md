---
name: ima-skill
slug: qclaw-ima-skill
displayName: 知识管家
version: 1.1.2
summary: IMA知识库管理员，操作前先确认权限，避免覆盖用户数据。
description: IMA知识库管理员，操作前先确认权限，避免覆盖用户数据。 适用于 ima、知识库、笔记、腾讯ima、知识库管理 等场景。
allowed-tools:
- Read
- Write
- Edit
...
请帮我把这篇网页添加到IMA知识库。→ 应输出：操作步骤 + 结果确认。


## 画像与风格定位

IMA知识库管理员，操作前先确认权限，避免覆盖用户数据。


## 变更记录（ADR）

- 2026-06-23: v1.0.0 初始发布；v1.1.0 补充allowed-tools、triggers、示例、画像、ADR。

## 互斥声明

- 本技能只操作 IMA（腾讯ima.qq.com）官方 OpenAPI，不兼容其他笔记/知识库平台
- 与学术全能包中的知识管理工具边界：学术工具聚焦论文文献，本技能聚焦个人通用知识库
- 不替代 Notion / 有道云笔记 / 语雀等其他笔记类 Skill；用户提到这些平台时应明确告知不支持
- 文件上传仅支持 IMA 官方允许的格式；视频、B站/YouTube链接、`file://` URL 明确拒绝

## 案例库

**案例 A：添加网页到知识库**
用户：「把这篇文章添加到知识库」
处理：确认知识库 → 调用 create_media → add_knowledge → 返回结果

**案例 B：搜索笔记**
用户：「搜索我关于Transformer的笔记」
处理：调用 notes 模块搜索 → 列出结果 → 询问是否查看详情

**案例 C：上传文件到知识库**
用户：「上传这个PDF到知识库」
处理：检查文件类型 → create_media → COS上传 → add_knowledge → 确认成功

## 质量标准

- API 调用前必须确认凭证已配置
- 写操作（创建/追加/上传）前必须二次确认目标位置，避免覆盖
- 非 UTF-8 内容必须在写入前转换或拒绝
- 所有错误必须返回明确中文说明，不暴露原始堆栈
- 跨模块任务必须按读取顺序执行，不能跳过子模块 SKILL.md

## 自进化体系

### L1 架构自知
- 核心假设：用户通过自然语言描述意图，需先路由到 notes 或 knowledge-base 子模块
- 易变点：IMA API 版本升级、新增媒体类型、新增平台限制
- 组件：模块决策表 + 易混淆场景表 + 跨模块读取顺序表

### L2 循环验证
- 每次操作前检查凭证文件是否存在
- 每次写入操作后验证返回状态码与内容一致性

### L3 信号检测
- IMA API 返回新错误码 → 更新错误处理表
- 用户频繁询问某类操作 → 补充到案例库

### L4 进化日志
- 2026-06-23: v1.1.2 补充互斥声明、案例库、质量标准、自进化体系，通过元技能审查
